...
for (const row of data) {
  ...
  const block = `${start}\n;(function(){var db=window.${owner},drug;if(Array.isArray(db)){var matches=db.filter(function(item){return item&&item.id===${JSON.stringify(id)};});if(matches.length!==1)throw new Error(${JSON.stringify(`GOLD33_CANONICAL_CARDINALITY:${id}:`)}+matches.length);drug=matches[0];}else{drug=db&&db[${JSON.stringify(id)}];if(!drug)throw new Error(${JSON.stringify(`GOLD33_MISSING_CANONICAL:${id}`)});}drug.mcGoldClinicalV1=${JSON.stringify(payload,null,2)};})();\n${end}\n`;
